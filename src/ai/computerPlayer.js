import { getLegalMoves, movePiece, dropPiece, isKingInCheck, PLAYER_1, PLAYER_2, PAWN, LANCE, KNIGHT, SILVER, GOLD, BISHOP, ROOK, KING, PROMOTED_PAWN, PROMOTED_LANCE, PROMOTED_KNIGHT, PROMOTED_SILVER, PROMOTED_BISHOP, PROMOTED_ROOK } from '../game/engine';
...
export function getAiMove(gameState, difficulty) {
  ...
  switch (difficulty) {
    ...
    case 'medium':
      ...
      return bestMove;
    case 'hard':
      let hardBestMove = null;
      let hardBestScore = -Infinity;

      const hardPossibleMoves = [];
      ...
      for (let r = 0; r < 9; r++) {
        for (let c = 0; c < 9; c++) {
          ...
          if (piece && piece.player === currentPlayer) {
            const moves = getLegalMoves(piece, r, c, gameState.board);
            moves.forEach(to => {
              hardPossibleMoves.push({ from: [r, c], to, type: 'move' });
            });
          }
        }
      }
        ...
        for (let r = 0; r < 9; r++) {
          for (let c = 0; c < 9; c++) {
            if (!gameState.board[r][c]) { // Only drop on empty squares
              const tempState = dropPiece(gameState, capturedPiece.type, [r, c]);
              if (tempState !== gameState) { // If drop was legal
                hardPossibleMoves.push({ from: 'drop', to: [r, c], type: capturedPiece.type });
              }
            }
          }
        }
      ...
      for (const move of hardPossibleMoves) {
        let newGameState;
        if (move.from === 'drop') {
          newGameState = dropPiece(gameState, move.type, move.to);
        } else {
