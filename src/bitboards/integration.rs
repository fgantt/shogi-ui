...
use crate::bitboards::{
    ...
    debruijn::{bit_scan_forward_debruijn, bit_scan_reverse_debruijn, get_all_bit_positions_debruijn},
    lookup_tables::{bit_positions_4bit_lookup, popcount_4bit_optimized},
    masks::{get_rank_mask, get_file_mask, get_diagonal_mask},
};
...
mod tests {
    ...
    fn test_popcount_consistency() {
        ...
        for bb in test_cases {
            ...
            assert_eq!(result1, global_result, "Global optimizer inconsistent for 0x{:X}", bb);
        }
    }
    ...
    fn test_bit_scan_consistency() {
        let optimizer = BitScanningOptimizer::new();
        let test_cases = [
            1u128, 2u128, 4u128, 8u128, 0xFFu128,
            0x8000000000000000u128, 0x10000000000000000u128,
            0x5555555555555555u128, 0x123456789ABCDEF0u128,
        ];
        ...
        for bb in test_cases {
            ...
            assert_eq!(forward1, global_forward, "Global forward scan inconsistent for 0x{:X}", bb);
            assert_eq!(reverse1, global_reverse, "Global reverse scan inconsistent for 0x{:X}", bb);
        }
    }
    ...
    fn test_analyze_bitboard() {
        ...
        // Test global optimizer
        let (global_popcount, global_first_bit, global_last_bit) = GlobalOptimizer::analyze_bitboard(bb);
