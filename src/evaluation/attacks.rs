...
impl AttackTables {
    ...
    fn initialize_tables(&mut self) {
        ...
        let piece_types = vec![
            ...
            PieceType::Gold, PieceType::Knight, PieceType::Lance, PieceType::Pawn,
    ...
    fn generate_piece_attacks(&self, piece_type: PieceType, pos: Position) -> Bitboard {
        ...
        match piece_type {
            ...
            PieceType::Gold => {
                ...
                self.add_single_attacks(&mut attacks, pos, &[
    ...
    fn add_single_attacks(&self, attacks: &mut Bitboard, start_pos: Position, directions: &[(i8, i8)]) {
    ...
    pub fn get_piece_attacks(&self, piece_type: PieceType, pos: Position) -> Bitboard {
        self.piece_attacks.get(&(piece_type, pos)).copied().unwrap_or(EMPTY_BITBOARD)
    }
    
    /// Get king zone for a position
    pub fn get_king_zone(&self, pos: Position) -> Bitboard {
        self.king_zones.get(&pos).copied().unwrap_or(EMPTY_BITBOARD)
    }
}
...
impl AttackAnalyzer {
    ...
    /// Analyze attacks in the king zone
    fn analyze_king_zone_attacks(&self, board: &BitboardBoard, king_pos: Position, king_zone: Bitboard, opponent: Player, evaluation: &mut AttackEvaluation) {
        for row in 0..9 {
            for col in 0..9 {
                let pos = Position::new(row, col);
                ...
                if let Some(piece) = board.get_piece(pos) {
                    if piece.player == opponent {
                        ...
                        if is_bit_set(piece_attacks, king_pos) {
                            ...
                            evaluation.attack_weight += self.get_piece_attack_value(piece.piece_type);
                        }
                    }
                }
            }
    ...
    fn analyze_attack_coordination(&self, board: &BitboardBoard, king_pos: Position, opponent: Player, evaluation: &mut AttackEvaluation) {
        let mut rook_attacks = EMPTY_BITBOARD;
        let mut bishop_attacks = EMPTY_BITBOARD;
                                    ...
                                    if is_bit_set(piece_attacks, target_pos) {
                                        ...
                                        for other_row in 0..9 {
                                            for other_col in 0..9 {
                                                ...
                                                if let Some(other_piece) = board.get_piece(other_pos) {
                                                    if other_piece.player == opponent && other_pos != pos {
                                                        let other_attacks = self.attack_tables.get_piece_attacks(other_piece.piece_type, other_pos);
        ...
        if coordination_squares != EMPTY_BITBOARD {
            evaluation.coordination_bonus += self.config.coordination_bonus;
        }
        
        // Double attack bonus
        if double_attacks > 0 {
            evaluation.coordination_bonus += self.config.double_attack_bonus * double_attacks as i32;
        }
    }
    ...
    fn convert_to_tapered_score(&self, evaluation: AttackEvaluation) -> TaperedScore {
        let total_score = evaluation.attack_weight + evaluation.coordination_bonus + evaluation.tactical_threats;
...
impl ThreatEvaluator {
    ...
    fn detect_line_pins(&self, board: &BitboardBoard, king_pos: Position, opponent: Player, piece_type: PieceType) -> i32 {
        let mut pins = 0;
        ...
        let directions = match piece_type {
            ...
            PieceType::Bishop | PieceType::PromotedBishop => vec![(1, 1), (-1, 1), (1, -1), (-1, -1)],
            _ => return 0,
        };
    ...
    fn detect_line_skewers(&self, board: &BitboardBoard, king_pos: Position, opponent: Player, piece_type: PieceType) -> i32 {
        let mut skewers = 0;
