...
pub struct CastleRecognizer {
    ...
    pattern_cache: std::cell::RefCell<std::collections::HashMap<(u64, Player, Position), Option<usize>>>,
...
impl CastleRecognizer {
    ...
    pub fn evaluate_castle_structure(&self, board: &BitboardBoard, player: Player, king_pos: Position) -> TaperedScore {
        // Check cache first
        let board_hash = self.get_board_hash(board);
        if let Some(cached_pattern) = self.pattern_cache.borrow().get(&(board_hash, player, king_pos)) {
        ...
        for (pattern_index, pattern) in self.patterns.iter().enumerate() {
            let (matches, required_matches, total_weight) = self.analyze_pattern_match(board, player, king_pos, pattern);
            ...
            let match_quality = self.calculate_match_quality(matches, pattern.pieces.len(), total_weight, pattern);
    ...
    fn matches_pattern(&self, board: &BitboardBoard, player: Player, king_pos: Position, pattern: &CastlePattern) -> bool {
        let (matches, required_matches, _) = self.analyze_pattern_match(board, player, king_pos, pattern);
        let required_pieces = pattern.pieces.iter().filter(|p| p.required).count();
    ...
    fn analyze_pattern_match(&self, board: &BitboardBoard, player: Player, king_pos: Position, pattern: &CastlePattern) -> (usize, usize, u32) {
        let mut matches = 0;
    ...
    fn get_relative_position(&self, king_pos: Position, relative_pos: (i8, i8), player: Player) -> Option<Position> {
    ...
    fn calculate_match_quality(&self, matches: usize, total_pieces: usize, total_weight: u32, pattern: &CastlePattern) -> f32 {
        if total_pieces == 0 {
            return 0.0;
        }
        ...
        let weight_quality = if max_weight > 0 { total_weight as f32 / max_weight as f32 } else { 0.0 };
...
mod tests {
    ...
    fn test_castle_recognition_both_players() {
        ...
        let black_score = recognizer.evaluate_castle_structure(&board, Player::Black, black_king_pos);
        
        // Test White king position
        let white_king_pos = Position::new(0, 4);
        let white_score = recognizer.evaluate_castle_structure(&board, Player::White, white_king_pos);
