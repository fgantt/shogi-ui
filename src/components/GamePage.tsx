...
const GamePage = () => {
  ...
  useEffect(() => {
    ...
    const importBoardBackgrounds = async () => {
      ...
      if (paths.length > 0) {
        ...
        // Use a more robust selector for the board
        const boardElement = document.querySelector('.board, .shogi-board');
        if (boardElement) {
          boardElement.style.backgroundImage = `url('${initialBoardBackground}')`;
        }
  ...
  const setRandomBoardBackground = () => {
    if (boardBackgroundList.length > 0) {
      ...
      const boardElement = document.querySelector('.board, .shogi-board');
      if (boardElement) {
        boardElement.style.backgroundImage = `url('${newBoardBackground}')`;
      }
  ...
  const handleSelectBoardBackground = (boardBackground) => {
    const boardElement = document.querySelector('.board, .shogi-board');
    if (boardElement) {
      boardElement.style.backgroundImage = `url('${boardBackground}')`;
    }
  ...
  return (
    <div className={`game-page ${isGameOver ? 'game-over' : ''}`}>
      ...
      <div className="game-container">
        <div className="board-and-captured-pieces">
          <CapturedPieces
