...
impl AdaptiveConfigurationManager {
    ...
    pub fn new(initial_config: TranspositionConfig) -> Self {
        let runtime_manager = Arc::new(Mutex::new(RuntimeConfigurationManager::new(initial_config)));
    ...
    fn add_default_rules(&mut self) {
        ...
        self.add_rule(AdaptationRule {
            ...
            action: AdaptationAction::IncreaseTableSize { percentage: 0.5, max_size: Some(1048576) },
            priority: 10,
            enabled: true,
        });
        ...
        self.add_rule(AdaptationRule {
            ...
            condition: AdaptationCondition::MemoryUsageAbove { threshold_bytes: 134217728, duration_ms: 5000 }, // 128MB
        ...
        self.add_rule(AdaptationRule {
            ...
            condition: AdaptationCondition::CollisionRateAbove { threshold: 0.15, duration_ms: 8000 },
            action: AdaptationAction::ChangeReplacementPolicy { policy: ReplacementPolicy::AgeBased },
            priority: 8,
            enabled: true,
        });
        ...
        self.add_rule(AdaptationRule {
            ...
            condition: AdaptationCondition::And(vec![
                ...
                AdaptationCondition::AvailableMemoryBelow { threshold_bytes: 268435456, duration_ms: 1000 }, // 256MB - inverted logic
            ]),
            action: AdaptationAction::SwitchToTemplate { template_name: "high_performance".to_string() },
            priority: 6,
            enabled: true,
        });
    }
    ...
    pub fn update_performance_metrics(&mut self, metrics: RuntimePerformanceMetrics) -> Result<(), String> {
    ...
    fn should_adapt(&self) -> bool {
        ...
        self.adaptation_rules.iter().any(|rule| {
            rule.enabled && self.evaluate_condition(&rule.condition)
        })
    }
    ...
    fn evaluate_condition(&self, condition: &AdaptationCondition) -> bool {
        match condition {
            ...
            AdaptationCondition::MemoryUsageAbove { threshold_bytes, duration_ms } => {
                self.check_metric_threshold(|m| m.memory_usage_bytes as f64, *threshold_bytes as f64, true, *duration_ms)
            }
            AdaptationCondition::OperationTimeAbove { threshold_us, duration_ms } => {
                self.check_metric_threshold(|m| m.avg_operation_time_us, *threshold_us, true, *duration_ms)
            }
            ...
            AdaptationCondition::AvailableMemoryBelow { threshold_bytes, duration_ms } => {
                self.check_metric_threshold(|m| m.available_memory_bytes as f64, *threshold_bytes as f64, false, *duration_ms)
            }
    ...
    fn check_metric_threshold<F>(&self, metric_extractor: F, threshold: f64, above: bool, duration_ms: u64) -> bool
    where
        F: Fn(&RuntimePerformanceMetrics) -> f64,
    {
        let _cutoff_time = std::time::SystemTime::now() - std::time::Duration::from_millis(duration_ms);
    ...
    fn perform_adaptation(&mut self) -> Result<(), String> {
        ...
        if let Some(rule) = self.adaptation_rules.iter().find(|rule| {
            rule.enabled && self.evaluate_condition(&rule.condition)
        }) {
    ...
    fn execute_action(&self, action: &AdaptationAction) -> Result<(), String> {
        ...
        let new_config = match action {
            ...
            AdaptationAction::SwitchToTemplate { template_name } => {
                runtime_manager.get_template(template_name)
                    .ok_or_else(|| format!("Template '{}' not found", template_name))?
                    .clone()
            }
            AdaptationAction::CustomConfiguration { config } => {
                config.clone()
            }
