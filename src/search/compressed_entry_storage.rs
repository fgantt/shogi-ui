...
impl CompressedEntryStorage {
    ...
    pub fn compress_entry(&mut self, entry: &TranspositionEntry) -> CompressedEntry {
        ...
        self.stats.bytes_saved += (original_size as u64).saturating_sub(compressed_data.len() as u64);
    ...
    fn decompress_bit_packing(&self, data: &[u8]) -> Vec<u8> {
        ...
        decompressed.push(0); decompressed.push(0); // Pad hash key to 8 bytes
    ...
    fn cache_decompressed(&mut self, key: u64, data: &[u8]) {
        ...
        self.cache.insert(key, CompressedEntry {
            data: data.to_vec(),
