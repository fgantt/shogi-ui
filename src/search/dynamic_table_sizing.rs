...
impl DynamicTableSizer {
    ...
    pub fn record_performance(&mut self, hit_rate: f64, access_frequency: f64) {
        ...
        self.performance_stats.avg_hit_rate = total_hit_rate / self.performance_history.len() as f64;
    }
    ...
    fn update_memory_stats(&mut self) {
        ...
        let recent_samples: Vec<u64> = self.memory_history.iter()
            .rev()
            .take(10)
            .map(|(_, usage)| *usage)
            .collect();
        ...
        if recent_samples.len() >= 3 {
            let first_half: f64 = recent_samples[..recent_samples.len()/2].iter().sum::<u64>() as f64;
            let second_half: f64 = recent_samples[recent_samples.len()/2..].iter().sum::<u64>() as f64;
    ...
    fn update_performance_stats(&mut self) {
        ...
        let recent_samples: Vec<f64> = self.performance_history.iter()
            .rev()
            .take(10)
            .map(|(_, rate)| *rate)
            .collect();
    ...
    fn analyze_temporal_patterns(&mut self) {
        ...
        if total_periods > 0 {
            self.access_analysis.temporal_patterns.burst_frequency = bursts as f64 / total_periods as f64;
            self.access_analysis.temporal_patterns.quiet_frequency = quiet_periods as f64 / total_periods as f64;
        }
        ...
        if !time_diffs.is_empty() {
            ...
            let variance: f64 = time_diffs.iter()
                .map(|diff| {
    ...
    fn make_resize_decision(&self) -> Option<ResizeDecision> {
        ...
        if self.config.memory_monitoring.enable_memory_pressure &&
           self.memory_stats.pressure_level > self.config.memory_monitoring.memory_pressure_threshold {
        ...
        if self.config.performance_monitoring.enable_hit_rate_monitoring {
            if self.performance_stats.hit_rate < self.config.performance_monitoring.hit_rate_threshold {
                reasons.push(ResizeReason::LowHitRate);
                confidence += 0.6;
                
                // Upsize to improve hit rate
                new_size = (new_size as f64 * 1.2) as usize;
            } else if self.performance_stats.hit_rate > self.config.performance_monitoring.hit_rate_threshold + 0.1 {
