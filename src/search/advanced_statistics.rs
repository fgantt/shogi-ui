...
impl CollisionMonitor {
    ...
    pub fn get_collision_stats(&self) -> CollisionStats {
        ...
        let variance = histogram.iter()
            ...
            .sum::<f64>() / self.table_size as f64;
        ...
        CollisionStats {
            ...
            most_collided_hashes: collision_map.iter()
...
impl StatisticsExporter {
    ...
    fn export_text(&self, stats: &DetailedCacheStats) -> String {
        if self.include_details {
            format!(
                ...
                stats.total_probes, stats.total_hits, stats.total_stores, stats.total_replacements,
                stats.total_collisions, stats.occupancy_rate * 100.0, stats.avg_probe_time_us,
                stats.avg_store_time_us, stats.memory_usage_bytes, stats.hash_distribution_quality
            )
        } else {
            format!(
                ...
                Hit Rate: {:.2}%\n\
                Memory Usage: {} bytes",
                stats.total_probes, stats.total_hits,
                if stats.total_probes > 0 { stats.total_hits as f64 / stats.total_probes as f64 * 100.0 } else { 0.0 },
    ...
    fn export_hit_rates_json(&self, hit_rates: &HitRateByDepth) -> String {
        ...
        let json_entries: Vec<String> = data.iter()
    ...
    fn export_hit_rates_text(&self, hit_rates: &HitRateByDepth) -> String {
        ...
        text.push_str(&format!("Total Hit Rate: {:.2}%\n\n", hit_rates.get_total_hit_rate() * 100.0));
        ...
        if self.include_details {
            ...
            if !rates.is_empty() {
                let avg_rate = rates.iter().map(|(_, rate)| *rate).sum::<f64>() / rates.len() as f64;
                let min_rate = rates.iter().map(|(_, rate)| *rate).fold(f64::INFINITY, |a, b| a.min(b));
                let max_rate = rates.iter().map(|(_, rate)| *rate).fold(0.0f64, |a, b| a.max(b));
                text.push_str(&format!("Average: {:.2}%, Min: {:.2}%, Max: {:.2}%\n", 
                    avg_rate * 100.0, min_rate * 100.0, max_rate * 100.0));
            }
        }
        text
    }
    ...
    "most_collided_hashes": [{}]
            ...
            stats.total_collisions, stats.unique_collision_hashes, stats.max_collisions_per_hash,
            stats.hash_distribution_quality,
            stats.most_collided_hashes.iter()
    ...
    fn export_collision_text(&self, stats: &CollisionStats) -> String {
        if self.include_details {
            ...
            format!(
                ...
                Unique Collision Hashes: {}\n\
                Hash Distribution Quality: {:.4}",
                stats.total_collisions, stats.unique_collision_hashes, stats.hash_distribution_quality
            )
        }
    }
...
impl PerformanceTrendAnalyzer {
    ...
    pub fn analyze_trends(&self) -> PerformanceTrends {
        ...
        let probe_time_volatility = self.calculate_volatility(&recent_points, |p| p.avg_probe_time_us);
        
        // Calculate performance score (higher is better)
        let recent_avg_hit_rate = recent_points.iter().map(|p| p.hit_rate).sum::<f64>() / n as f64;
        let recent_avg_probe_time = recent_points.iter().map(|p| p.avg_probe_time_us).sum::<f64>() / n as f64;
...
impl AdvancedStatisticsManager {
    ...
    pub fn record_probe(&self, depth: u8, hit: bool, probe_time_us: f64) {
        ...
        stats.avg_probe_time_us = (stats.avg_probe_time_us * (total_probes - 1.0) + probe_time_us) / total_probes;
    ...
    pub fn record_store(&self, store_time_us: f64, replacement: bool) {
        ...
        stats.avg_store_time_us = (stats.avg_store_time_us * (total_stores - 1.0) + store_time_us) / total_stores;
    }
