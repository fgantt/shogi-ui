...
impl EngineManager {
    ...
    /// Initialize an engine with the USI protocol and send saved options
    pub async fn initialize_engine_with_options(&self, engine_id: &str, engine_storage: &tokio::sync::RwLock<crate::engine_storage::EngineStorage>) -> Result<()> {
        log::info!("Initializing engine with options: {}", engine_id);
