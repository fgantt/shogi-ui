...
use shogi_engine::{
    ShogiEngine, BitboardBoard, Player, CapturedPieces, Position, PieceType,
...
fn custom_configuration_example() {
    ...
    config.performance.eviction_strategy = shogi_engine::tablebase::tablebase_config::EvictionStrategy::LRU;
...
fn memory_management_example() {
    ...
    for i in 0..50 {
        ...
        if i % 10 == 0 {
            ...
            println!("  After {} probes: current={} bytes, peak={} bytes", 
                    i + 1, current_memory, peak_memory);
        }
    }
...
fn adaptive_solver_selection_example() {
    ...
    for priority in [90, 80, 70, 50] {
        let suitable = tablebase.is_position_suitable_for_solver(&board, Player::Black, &captured_pieces, priority);
