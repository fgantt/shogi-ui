...
fn demonstrate_warming_strategies() {
    ...
    for (name, config) in strategies {
        ...
        println!("  Results: {} entries warmed in {}μs", 
...
fn demonstrate_warming_sessions() {
    ...
    println!("Session 1: {} entries warmed, {:.1}% success rate",
        results1.total_entries, results1.success_rate * 100.0);
    ...
    println!("Session 2: {} entries warmed, {:.1}% success rate",
        results2.total_entries, results2.success_rate * 100.0);
    ...
    println!("Session 3: {} entries warmed, {:.1}% success rate",
        results3.total_entries, results3.success_rate * 100.0);
    
    // Show session history
    let recent_sessions = warmer.get_recent_sessions(3);
    println!("Recent sessions:");
    for session in recent_sessions {
        println!("  Session {}: {} entries, {:?} strategy",
...
fn demonstrate_warming_performance() {
    ...
    for (name, config) in configs {
        ...
        let memory_efficiency = results.total_entries as f64 / (results.memory_used as f64 / 1024.0);
        let time_efficiency = results.total_entries as f64 / (results.warming_time_us as f64 / 1000.0);
...
use crate::types::{TranspositionEntry, TranspositionFlag, Move, Position, PieceType, Player};
