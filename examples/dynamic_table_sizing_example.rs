...
use shogi_engine::search::{
    ...
    MemoryStats, PerformanceStats, AccessPatternAnalysis
};
...
fn demonstrate_memory_pressure_handling() {
    ...
    for i in 0..5 {
        ...
        if let Some(decision) = sizer.should_resize() {
            println!("  Memory usage: {} -> Resize decision: {:?} (size: {})", 
    ...
    if let Some(decision) = sizer.should_resize() {
        println!("  Low memory pressure -> Resize decision: {:?} (size: {})", 
...
fn demonstrate_performance_based_resizing() {
    ...
    if let Some(decision) = sizer.should_resize() {
        println!("  Hit rate: {:.1}% -> Resize decision: {:?} (size: {})", 
            sizer.performance_stats.hit_rate * 100.0, decision.reason, decision.new_size);
        sizer.apply_resize(&decision);
    }
    ...
    if let Some(decision) = sizer.should_resize() {
        println!("  Performance degrading -> Resize decision: {:?} (size: {})", 
            decision.reason, decision.new_size);
        sizer.apply_resize(&decision);
    }
...
fn demonstrate_access_pattern_analysis() {
    ...
    if let Some(decision) = sizer.should_resize() {
        println!("  Access pattern -> Resize decision: {:?} (size: {})", 
            decision.reason, decision.new_size);
    }
}
...
fn demonstrate_realtime_adaptation() {
    ...
    for period in 0..10 {
        ...
        if let Some(decision) = sizer.should_resize() {
            ...
            println!("  Memory: {}, Hit rate: {:.1}%, Size: {} -> {} ({:+.1}%)",
                memory_usage, hit_rate * 100.0, size_before, decision.new_size, size_change);
            println!("  Reason: {:?}, Confidence: {:.2}", decision.reason, decision.confidence);
        } else {
            println!("  Memory: {}, Hit rate: {:.1}%, Size: {} (no change)",
                memory_usage, hit_rate * 100.0, size_before);
        }
    }
    ...
    println!("  Average size change: {:.1}%", total_size_changes as f64 / resize_count.max(1) as f64);
