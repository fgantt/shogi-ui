...
fn benchmark_material_balance(c: &mut Criterion) {
    ...
    group.bench_function("calculate_balance", |b| {
        let mut evaluator = MaterialEvaluator::new();
        b.iter(|| {
            black_box(evaluator.calculate_material_balance(&board, &captured_pieces, Player::Black));
        });
    });
...
fn benchmark_configurations(c: &mut Criterion) {
    ...
    let configs = vec![
        ...
        ("no_hand_pieces", MaterialEvaluationConfig {
...
fn benchmark_memory_patterns(c: &mut Criterion) {
    ...
    group.bench_function("create_many_evaluators", |b| {
        b.iter(|| {
            let evaluators: Vec<MaterialEvaluator> = (0..100)
                .map(|_| MaterialEvaluator::new())
                .collect();
