...
fn benchmark_move_scoring_positions(c: &mut Criterion) {
    ...
    for position in positions {
        group.bench_with_input(BenchmarkId::new("position", format!("{:?}", position)), &position, |b, &position| {
            let mut orderer = MoveOrdering::new();
            let moves = generate_test_moves(100);
...
fn benchmark_move_scoring_weights(c: &mut Criterion) {
    ...
    let weight_configs = vec![
        ...
        ("high_capture", OrderingWeights {
            capture_weight: 2000,
            ..Default::default()
        }),
        ("high_tactical", OrderingWeights {
            tactical_weight: 600,
            ..Default::default()
        }),
        ("balanced", OrderingWeights {
            capture_weight: 1500,
...
fn benchmark_move_scoring_players(c: &mut Criterion) {
    ...
    for player in players {
        group.bench_with_input(BenchmarkId::new("player", format!("{:?}", player)), &player, |b, &player| {
            let mut orderer = MoveOrdering::new();
            let moves = generate_test_moves(100);
...
fn benchmark_move_scoring_cache_hit_rates(c: &mut Criterion) {
    ...
    for hit_rate in hit_rates {
        group.bench_with_input(BenchmarkId::new("hit_rate", format!("{:.0}%", hit_rate)), &hit_rate, |b, &hit_rate| {
            let mut orderer = MoveOrdering::new();
            let moves = generate_test_moves(100);
