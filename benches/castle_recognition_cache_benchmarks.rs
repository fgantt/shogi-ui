...
fn create_mino_castle_board(player: Player, king_row: u8, king_col: u8) -> (BitboardBoard, Position) {
...
fn create_anaguma_castle_board(player: Player, king_row: u8, king_col: u8) -> (BitboardBoard, Position) {
    let mut board = BitboardBoard::empty();
    let king_pos = Position::new(king_row, king_col);
...
fn relative_pos(king: Position, player: Player, rank_delta: i8, file_delta: i8) -> Option<Position> {
