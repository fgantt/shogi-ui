...
fn bench_cache_operations(c: &mut Criterion) {
    ...
    group.bench_function("probe_miss", |b| {
        b.iter(|| {
            black_box(cache.probe(&board, Player::Black, &captured_pieces))
        })
    });
    ...
    group.bench_function("store", |b| {
        b.iter(|| {
            cache.store(&board, Player::Black, &captured_pieces, black_box(150), 5)
        })
    });
...
fn bench_replacement_policies(c: &mut Criterion) {
    ...
    for policy in policies {
        ...
        group.bench_with_input(
            ...
            |b, _| {
                b.iter(|| {
                    cache.store(&board, Player::Black, &captured_pieces, black_box(150), 6)
                })
            },
        );
    }
...
fn bench_cache_load_patterns(c: &mut Criterion) {
    ...
    group.bench_function("random_access", |b| {
        b.iter(|| {
            for depth in 1..=100 {
                cache.store(&board, Player::Black, &captured_pieces, depth as i32 * 10, depth as u8 % 10);
                let _ = black_box(cache.probe(&board, Player::Black, &captured_pieces));
            }
        })
    });
...
fn bench_statistics_overhead(c: &mut Criterion) {
    ...
    group.bench_function("probe_with_stats", |b| {
        b.iter(|| {
            black_box(cache_with_stats.probe(&board, Player::Black, &captured_pieces))
        })
    });
    ...
    group.bench_function("probe_without_stats", |b| {
        b.iter(|| {
            black_box(cache_no_stats.probe(&board, Player::Black, &captured_pieces))
        })
    });
...
fn bench_verification_overhead(c: &mut Criterion) {
    ...
    group.bench_function("probe_with_verify", |b| {
        b.iter(|| {
            black_box(cache_with_verify.probe(&board, Player::Black, &captured_pieces))
        })
    });
    ...
    group.bench_function("probe_without_verify", |b| {
        b.iter(|| {
            black_box(cache_no_verify.probe(&board, Player::Black, &captured_pieces))
        })
    });
...
fn bench_get_statistics(c: &mut Criterion) {
    ...
    c.bench_function("get_statistics", |b| {
        b.iter(|| {
            black_box(cache.get_statistics())
        })
    });
}
