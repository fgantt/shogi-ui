...
use shogi_engine::{
    search::move_ordering::{MoveOrdering, MoveOrderingConfig, CacheEvictionPolicy},
    bitboards::{BitboardBoard, Position, PieceType, Player, CapturedPieces},
...
fn benchmark_cache_eviction_policies(c: &mut Criterion) {
    ...
    for (name, policy) in policies {
        ...
        group.bench_with_input(
            BenchmarkId::new("eviction_policy", name),
            &name,
            |b, _| {
                b.iter(|| {
...
fn benchmark_history_heuristic_enhancements(c: &mut Criterion) {
    ...
    for (name, enable_relative, enable_quiet, enable_phase) in configs {
        ...
        group.bench_with_input(
            BenchmarkId::new("history_config", name),
            &name,
            |b, _| {
                b.iter(|| {
                    let ordered = orderer.order_moves_with_all_heuristics(
