...
use shogi_engine::{
    ...
    types::{CapturedPieces, Player, IIDConfig, NullMoveConfig},
};
...
fn benchmark_algorithm_skip_rates(c: &mut Criterion) {
    ...
    group.bench_function("skip_rate_high_pressure_10ms", |b| {
        b.iter(|| {
            ...
            let iid_skip_rate = if iid_stats.iid_searches_performed + iid_stats.positions_skipped_time_pressure > 0 {
                iid_stats.positions_skipped_time_pressure as f64 /
                (iid_stats.iid_searches_performed + iid_stats.positions_skipped_time_pressure) as f64
            } else {
                0.0
            };
