...
fn benchmark_history_score_update(c: &mut Criterion) {
    ...
    for count in [10, 50, 100, 500, 1000].iter() {
        group.bench_with_input(BenchmarkId::new("update_history_score", count), count, |b, &count| {
...
fn benchmark_history_move_scoring(c: &mut Criterion) {
    ...
    for count in [10, 50, 100, 500, 1000].iter() {
        group.bench_with_input(BenchmarkId::new("score_history_move", count), count, |b, &count| {
            let mut orderer = MoveOrdering::new();
            let moves = generate_test_moves(count);
...
fn benchmark_history_move_ordering(c: &mut Criterion) {
    ...
    for count in [10, 50, 100, 500, 1000].iter() {
        group.bench_with_input(BenchmarkId::new("order_moves_with_history", count), count, |b, &count| {
            let mut orderer = MoveOrdering::new();
            let moves = generate_test_moves(count);
...
fn benchmark_history_heuristic_configuration(c: &mut Criterion) {
    ...
    for max_score in [1000, 5000, 10000, 20000, 50000].iter() {
        group.bench_with_input(BenchmarkId::new("max_history_score", max_score), max_score, |b, &max_score| {
            let mut orderer = MoveOrdering::new();
...
fn benchmark_history_heuristic_statistics(c: &mut Criterion) {
    ...
    for count in [10, 50, 100, 500, 1000].iter() {
        group.bench_with_input(BenchmarkId::new("statistics_tracking", count), count, |b, &count| {
            let mut orderer = MoveOrdering::new();
            let moves = generate_test_moves(count);
...
fn benchmark_history_heuristic_cleanup(c: &mut Criterion) {
    ...
    for count in [10, 50, 100, 500, 1000].iter() {
        group.bench_with_input(BenchmarkId::new("clear_history_table", count), count, |b, &count| {
            let mut orderer = MoveOrdering::new();
            let moves = generate_test_moves(count);
...
fn benchmark_history_heuristic_all_integration(c: &mut Criterion) {
    ...
    for count in [10, 50, 100, 500, 1000].iter() {
        group.bench_with_input(BenchmarkId::new("order_moves_with_all_heuristics", count), count, |b, &count| {
            let mut orderer = MoveOrdering::new();
            ...
            // Store PV move
            if !moves.is_empty() {
                orderer.update_pv_move(&board, &captured_pieces, player, depth, moves[0].clone(), 100);
            }
...
fn benchmark_history_heuristic_piece_types(c: &mut Criterion) {
    ...
    for piece_type in piece_types {
        group.bench_with_input(BenchmarkId::new("piece_type", format!("{:?}", piece_type)), &piece_type, |b, &piece_type| {
            let mut orderer = MoveOrdering::new();
            let moves = generate_test_moves(100);
            
            // Create moves with specific piece type
            let specific_moves: Vec<Move> = moves.iter()
...
fn benchmark_history_heuristic_aging_factor(c: &mut Criterion) {
    ...
    for aging_factor in [0.5, 0.7, 0.8, 0.9, 0.95].iter() {
        group.bench_with_input(BenchmarkId::new("aging_factor", format!("{:.2}", aging_factor)), aging_factor, |b, &aging_factor| {
            let mut orderer = MoveOrdering::new();
