...
mod integration_coordination_tests {
    ...
    fn test_iid_move_ordering_lmr_coordination() {
        ...
        if iid_stats.iid_searches_performed > 0 {
            assert!(lmr_stats.iid_move_explicitly_exempted > 0,
    ...
    fn test_nmp_iid_hash_history_isolation() {
        ...
        assert!(result.is_some(), "Search should complete successfully with isolated hash histories");
    }
    ...
    fn test_all_algorithms_integration() {
        ...
        println!("TT hit rate: {:.2}%", if metrics.total_tt_probes > 0 {
...
mod integration_statistics_tests {
    ...
    fn test_edge_case_monitoring() {
        ...
        println!("IID moves reduced (edge case - should be 0): {}", lmr_stats.iid_move_reduced_count);
        println!("TT pollution prevented: {}", metrics.tt_auxiliary_overwrites_prevented);
