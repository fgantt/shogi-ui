...
use shogi_engine::evaluation::config::{ComponentFlagsForValidation, TaperedEvalConfig, WeightPreset};
...
fn test_weight_normalization() {
    ...
    assert!((original_ratio - new_ratio).abs() < 0.001, 
            "Ratios should be maintained: original={}, new={}", original_ratio, new_ratio);
    
    // Verify sum is now in range
    let sum = config.weights.material_weight + config.weights.position_weight 
            + config.weights.tactical_weight + config.weights.positional_weight;
    assert!(sum >= 5.0 && sum <= 15.0, 
            "Sum should be in range [5.0, 15.0]: sum={}", sum);
}
...
fn test_auto_normalize_weights() {
    ...
    let sum = config.weights.material_weight + config.weights.position_weight;
    assert!(sum >= 5.0 && sum <= 15.0, 
            "Weights should be normalized to range [5.0, 15.0]: sum={}", sum);
}
...
fn test_recommended_ranges() {
    ...
    assert!(warnings.is_empty(), 
            "Default config should have no range warnings. Found: {:?}", warnings);
}
