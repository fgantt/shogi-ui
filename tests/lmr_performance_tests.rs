...
fn should_run_performance_tests() -> bool {
    ...
    std::env::var("RUN_PERFORMANCE_TESTS").is_ok() || 
...
mod lmr_nps_comparison_tests {
    ...
    fn measure_search_performance(engine: &mut SearchEngine, board: &BitboardBoard, 
                                captured_pieces: &CapturedPieces, player: Player, 
                                depth: u8, iterations: usize) -> (f64, u64) {
    ...
    fn test_lmr_nps_improvement() {
        ...
        let (nps_with_lmr, nodes_with_lmr) = measure_search_performance(
            &mut engine_with_lmr, &board, &captured_pieces, player, depth, iterations
        );
        
        // Measure performance without LMR
        let (nps_without_lmr, nodes_without_lmr) = measure_search_performance(
            &mut engine_without_lmr, &board, &captured_pieces, player, depth, iterations
        );
    ...
    fn test_lmr_depth_improvement() {
        ...
        let start_time = Instant::now();
        let result_with_lmr = engine_with_lmr.search_at_depth(&board, &captured_pieces, player, 6, time_limit);
        let time_with_lmr = start_time.elapsed();
        
        let start_time = Instant::now();
        let result_without_lmr = engine_without_lmr.search_at_depth(&board, &captured_pieces, player, 6, time_limit);
...
mod lmr_regression_tests {
    ...
    fn test_lmr_tactical_accuracy() {
        ...
        let result_with_lmr = engine_with_lmr.search_at_depth(&board, &captured_pieces, player, 5, 2000);
        let result_without_lmr = engine_without_lmr.search_at_depth(&board, &captured_pieces, player, 5, 2000);
        ...
        assert!(score_diff < 1000, "Score difference too large: {} vs {}", score_with_lmr, score_without_lmr);
...
mod lmr_stress_tests {
    ...
    fn test_lmr_deep_search() {
        ...
        // Test deep search
        let result = engine.search_at_depth(&board, &captured_pieces, player, MAX_SEARCH_DEPTH, 10000);
        assert!(result.is_some());
