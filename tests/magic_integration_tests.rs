...
fn test_bitboard_with_magic_support() {
    ...
    assert!(result.is_ok(), "Failed to create BitboardBoard with magic support: {:?}", result.err());
...
fn test_magic_sliding_moves_generation() {
    ...
    if let Some(moves) = board.generate_magic_sliding_moves(rook_pos, PieceType::Rook, Player::Black) {
...
fn test_magic_vs_raycast_consistency() {
    ...
    let magic_moves = board.generate_magic_sliding_moves(bishop_pos, PieceType::Bishop, Player::White);
...
fn test_sliding_generator_respects_own_pieces() {
    ...
    // Generate moves
    if let Some(moves) = board.generate_magic_sliding_moves(bishop_pos, PieceType::Bishop, Player::Black) {
...
fn test_board_clone_preserves_magic_support() {
    ...
    assert_eq!(board1.has_magic_support(), board2.has_magic_support(),
...
fn test_magic_table_serialization_integration() {
    ...
    for square in (0..81).step_by(10) {
        ...
        assert_eq!(attacks1, attacks2, "Deserialized table should match original for square {}", square);
    }
}
...
fn test_multiple_pieces_with_magic() {
    ...
    board.place_piece(Piece { piece_type: PieceType::Bishop, player: Player::White }, Position::new(2, 2));
    board.place_piece(Piece { piece_type: PieceType::Rook, player: Player::White }, Position::new(4, 4));
    
    // Generate moves for each piece
    let rook1_moves = board.generate_magic_sliding_moves(Position::new(0, 0), PieceType::Rook, Player::Black);
    let bishop_moves = board.generate_magic_sliding_moves(Position::new(2, 2), PieceType::Bishop, Player::White);
    let rook2_moves = board.generate_magic_sliding_moves(Position::new(4, 4), PieceType::Rook, Player::White);
...
fn test_magic_table_validation() {
    ...
    assert!(validation_result.is_ok(), "Magic table validation failed: {:?}", validation_result.err());
}
