...
mod aspiration_window_performance_tests {
    ...
    fn test_aspiration_windows_performance_benchmark() {
        ...
        let start_time = Instant::now();
        let mut searcher = IterativeDeepening::new(4, 2000, None, None);
        let result_without_aspiration = searcher.search(&mut engine, &board, &captured_pieces, player);
    ...
    fn test_aspiration_windows_efficiency_metrics() {
        ...
        assert!(metrics.success_rate + metrics.fail_low_rate + metrics.fail_high_rate <= 1.1); // Allow for rounding
    }
    ...
    fn test_aspiration_windows_edge_case_performance() {
        ...
        let extreme_configs = vec![
            ("Very small window", AspirationWindowConfig {
                enabled: true,
                base_window_size: 10,
            ...
            ("Very large window", AspirationWindowConfig {
                enabled: true,
            ...
            ("High research limit", AspirationWindowConfig {
                enabled: true,
                base_window_size: 50,
                dynamic_scaling: true,
                max_window_size: 200,
