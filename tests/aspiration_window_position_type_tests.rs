...
fn test_window_size_calculation_with_stats() {
    ...
    for (depth, (prev_score, failures)) in depths.iter().zip(previous_scores.iter().zip(recent_failures.iter())) {
        let window_size = engine.calculate_window_size_with_stats(
            *depth, 
            *prev_score, 
            *failures
        );
    ...
    assert!(last_window_size >= 10 && last_window_size <= config.max_window_size,
...
fn test_aspiration_stats_with_phase() {
    ...
    for (phase, (window_size, research)) in phases.iter().zip(window_sizes.iter().zip(had_research.iter())) {
...
fn test_incremental_average_update() {
    ...
    assert!((stats.average_window_size - expected_avg).abs() < 0.01,
        "Expected average {}, got {}", expected_avg, stats.average_window_size);
}
...
fn test_config_defaults() {
    ...
    assert_eq!(config.enable_position_type_tracking, true, "enable_position_type_tracking should default to true");
    assert_eq!(config.disable_statistics_in_production, false, "disable_statistics_in_production should default to false");
}
