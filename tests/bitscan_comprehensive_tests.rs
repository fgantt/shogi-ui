...
use shogi_engine::types::{Bitboard, Position, Player};
...
mod unit_tests {
    ...
    fn test_popcount_correctness() {
        ...
        for (bb, expected) in test_cases {
            ...
            assert_eq!(popcount_cache_optimized(bb), expected, "Cache-optimized popcount failed for 0x{:X}", bb);
            
            // Test branch-optimized implementation
            assert_eq!(popcount_branch_optimized(bb), expected, "Branch-optimized popcount failed for 0x{:X}", bb);
            
            // Test critical path implementation
            assert_eq!(popcount_critical(bb), expected, "Critical path popcount failed for 0x{:X}", bb);
        }
    }
    ...
    fn test_bit_scan_forward_correctness() {
        ...
        for (bb, expected) in test_cases {
            // Test standard implementation
            assert_eq!(bit_scan_forward(bb), expected, "Standard bit_scan_forward failed for 0x{:X}", bb);
            
            // Test branch-optimized implementation
            assert_eq!(bit_scan_forward_optimized(bb), expected, "Branch-optimized bit_scan_forward failed for 0x{:X}", bb);
            
            // Test critical path implementation
            assert_eq!(bit_scan_forward_critical(bb), expected, "Critical path bit_scan_forward failed for 0x{:X}", bb);
        }
    }
    ...
    fn test_bit_scan_reverse_correctness() {
        ...
        for (bb, expected) in test_cases {
            // Test standard implementation
            assert_eq!(bit_scan_reverse(bb), expected, "Standard bit_scan_reverse failed for 0x{:X}", bb);
            
            // Test branch-optimized implementation
            assert_eq!(bit_scan_reverse_optimized(bb), expected, "Branch-optimized bit_scan_reverse failed for 0x{:X}", bb);
        }
    }
    ...
    fn test_platform_detection() {
        ...
        assert!(matches!(popcount_impl, PopcountImpl::Hardware | PopcountImpl::BitParallel | PopcountImpl::Software));
        assert!(matches!(bitscan_impl, BitscanImpl::Hardware | BitscanImpl::DeBruijn | BitscanImpl::Software));
    }
}
...
mod integration_tests {
    ...
    fn test_global_optimizer_integration() {
        ...
        assert_eq!(integration::GlobalOptimizer::get_all_bit_positions(bb), get_all_bit_positions(bb));
    }
...
mod performance_tests {
    ...
    fn benchmark_branch_prediction() {
        ...
        for &bb in &test_data[..10] {
            ...
            assert_eq!(bit_scan_forward_optimized(bb), if bb == 0 { None } else { Some(bb.trailing_zeros() as u8) });
...
mod cross_platform_tests {
    ...
    fn test_cross_platform_consistency() {
        ...
        for bb in test_cases {
            ...
            assert_eq!(standard_count, cache_count, "Cache-optimized popcount inconsistent for 0x{:X}", bb);
            assert_eq!(standard_count, branch_count, "Branch-optimized popcount inconsistent for 0x{:X}", bb);
            assert_eq!(standard_count, critical_count, "Critical path popcount inconsistent for 0x{:X}", bb);
            ...
            assert_eq!(standard_forward, branch_forward, "Branch-optimized bit_scan_forward inconsistent for 0x{:X}", bb);
            assert_eq!(standard_forward, critical_forward, "Critical path bit_scan_forward inconsistent for 0x{:X}", bb);
        }
    }
...
mod regression_tests {
    ...
    fn test_performance_regression() {
        ...
        assert!(current_time.as_millis() < 1000, "Performance regression detected: {:?}", current_time);
...
mod validation_tests {
    ...
    fn test_edge_cases_comprehensive() {
        ...
        for bb in edge_cases {
            ...
            assert_eq!(popcount_cache_optimized(bb), count, "Edge case cache popcount failed for 0x{:X}", bb);
            assert_eq!(popcount_branch_optimized(bb), count, "Edge case branch popcount failed for 0x{:X}", bb);
            assert_eq!(popcount_critical(bb), count, "Edge case critical popcount failed for 0x{:X}", bb);
            
            assert_eq!(bit_scan_forward(bb), forward, "Edge case bit_scan_forward failed for 0x{:X}", bb);
            assert_eq!(bit_scan_forward_optimized(bb), forward, "Edge case branch bit_scan_forward failed for 0x{:X}", bb);
            assert_eq!(bit_scan_forward_critical(bb), forward, "Edge case critical bit_scan_forward failed for 0x{:X}", bb);
            
            assert_eq!(bit_scan_reverse(bb), reverse, "Edge case bit_scan_reverse failed for 0x{:X}", bb);
            assert_eq!(bit_scan_reverse_optimized(bb), reverse, "Edge case branch bit_scan_reverse failed for 0x{:X}", bb);
        }
    }
