...
mod aspiration_window_performance_validation_tests {
    ...
    fn benchmark_aspiration_window_performance() {
        ...
        for _ in 0..iterations {
            ...
            let result = engine.search_at_depth(&board, &captured_pieces, player, 4, 100, -1000, 1000);
    ...
    fn benchmark_error_recovery_performance() {
        ...
        for _ in 0..iterations {
            let start = Instant::now();
            let result = engine.search_at_depth(&board, &captured_pieces, player, 4, 100, -2000, 2000);
            let elapsed = start.elapsed();
    ...
    fn test_memory_usage_stability() {
        ...
        for _ in 0..50 {
            let result = engine.search_at_depth(&board, &captured_pieces, player, 2, 50, -1000, 1000);
    ...
    fn test_error_handling_performance_impact() {
        ...
        assert!(ratio < 3.0, "Error handling should not slow normal case by more than 3x: {:.2}x", ratio);
    ...
    fn stress_test_rapid_searches() {
        ...
        for i in 0..100 {
            let result = engine.search_at_depth(&board, &captured_pieces, player, 2, 20, -1000, 1000);
            if result.is_some() {
                successful += 1;
            }
    ...
    fn stress_test_extreme_values() {
        ...
        for (alpha, beta) in extreme_cases {
            for _ in 0..10 {
                let result = engine.search_at_depth(&board, &captured_pieces, player, 2, 50, alpha, beta);
                if result.is_some() {
                    successful += 1;
                }
            }
        ...
        assert!(success_rate >= 90.0, "Extreme values test success rate too low: {:.1}%", success_rate);
    ...
    fn test_integration_performance() {
        ...
        assert!(success_rate >= 95.0, "Integration test success rate too low: {:.1}%", success_rate);
