...
mod tt_priority_tests {
    ...
    fn test_nmp_doesnt_overwrite_deeper_main_entry() {
        ...
        println!("TT auxiliary overwrites prevented: {}", metrics.tt_auxiliary_overwrites_prevented);
    ...
    fn test_entry_source_tagging() {
        use shogi_engine::types::{EntrySource, TranspositionEntry, TranspositionFlag, Move};
        ...
        let main_entry = TranspositionEntry::new(
            100, 5, TranspositionFlag::Exact, None, 0x12345, 0, EntrySource::MainSearch
        );
        assert_eq!(main_entry.source, EntrySource::MainSearch);
        
        let iid_entry = TranspositionEntry::new(
            50, 3, TranspositionFlag::Exact, None, 0x12345, 0, EntrySource::IIDSearch
        );
        assert_eq!(iid_entry.source, EntrySource::IIDSearch);
        
        let nmp_entry = TranspositionEntry::new(
            75, 4, TranspositionFlag::LowerBound, None, 0x12345, 0, EntrySource::NullMoveSearch
        );
