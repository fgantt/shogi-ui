...
fn test_repetition_detection_with_moves() {
    ...
    assert_eq!(hash_handler.get_repetition_state_for_hash(hash_after_unmake), RepetitionState::TwoFold);
}
...
fn test_repetition_detection_in_search_context() {
    ...
    if !moves.is_empty() {
        ...
        let hash_after_move = hash_handler.get_position_hash(&board, player.opposite(), &new_captured);
        hash_handler.add_position_to_history(hash_after_move);
