...
mod lmr_profiling_tests {
    ...
    fn test_lmr_profiling_overhead() {
        ...
        let profile_result = engine.profile_lmr_overhead(&board, &captured_pieces, player, depth, iterations);
...
mod lmr_performance_regression_tests {
    ...
    fn test_lmr_performance_consistency() {
        ...
        let avg_time: f64 = results.iter().map(|d| d.as_millis() as f64).sum::<f64>() / results.len() as f64;
    ...
    fn test_lmr_optimization_impact() {
        ...
        let result_with = engine_with_optimization.search_at_depth(&board, &captured_pieces, player, 4, 2000);
        let result_without = engine_without_optimization.search_at_depth(&board, &captured_pieces, player, 4, 2000);
        ...
        assert!(score_diff < 100, 
                "Score difference too large: {} vs {}", score_with, score_without);
