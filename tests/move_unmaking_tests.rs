...
use shogi_engine::{
    ...
    types::{CapturedPieces, Player, Position, PieceType, Move},
    moves::MoveGenerator,
};

fn board_fen_eq(board1: &BitboardBoard, board2: &BitboardBoard, captured1: &CapturedPieces, captured2: &CapturedPieces, player: Player) -> bool {
...
fn test_unmake_capture_move() {
    ...
    if let Some(test_move) = capture_move {
        ...
        assert_eq!(captured_before.black.len(), captured.black.len(), "Captured pieces should be restored");
    }
}
    ...
    if let Some(test_move) = promotion_move {
        ...
        if let Some(from) = move_info.from {
            if let Some(piece) = board.get_piece(from) {
                assert_eq!(piece.piece_type, move_info.original_piece_type, 
...
fn test_unmake_drop_move() {
    ...
    if let Some(test_move) = drop_move {
        ...
        assert_eq!(after_captured_count, initial_captured_count - 1, "Piece should be removed from hand");
        ...
        assert_eq!(restored_captured_count, initial_captured_count, "Piece should be returned to hand");
    }
}
...
fn test_multiple_moves_unmake() {
    ...
    assert_eq!(initial_fen, restored_fen, "Board should be fully restored after unmaking all moves");
}
...
fn test_move_info_structure() {
    ...
    assert_eq!(move_info.was_promotion, test_move.is_promotion, "MoveInfo should store promotion status");
...
fn test_unmake_in_search_context() {
    ...
    let after_fen = board.to_fen(player, &captured);
    assert_eq!(initial_fen, after_fen, "Board should be unchanged after search (move unmaking should restore state)");
