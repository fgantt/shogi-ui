...
use shogi_engine::types::{Player, PieceType, Position, Move, CapturedPieces};
...
fn test_drop_pressure_evaluation() {
    ...
    let score_without = evaluator_no_drop.evaluate_opening(&board, Player::Black, 5, Some(&captured), None);
...
fn test_drop_pressure_various_combinations() {
    ...
    let score_empty = evaluator.evaluate_opening(&board, Player::Black, 5, Some(&captured_empty), None);
    ...
    let score_many = evaluator.evaluate_opening(&board, Player::Black, 5, Some(&captured_many), None);
